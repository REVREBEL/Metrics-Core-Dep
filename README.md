...
`packages/ui` includes a registry metadata sync workflow for keeping the Metrics UI catalog aligned with component source files.
...
The sync workflow:
- Scans `packages/ui/src/components/**/*.tsx`
- Ignores stories, data, and internal-only directories
- Creates or updates metadata stubs for each component
- Preserves existing metadata entries where present
- Merges sibling `metadata.ts` files when they exist
- Writes the generated catalog metadata to `packages/ui/src/lib/registry.metadata.json`

`catalog/page.tsx` reads from `registry.metadata.json` when inline registry metadata is missing. The catalog is available at:
...
For richer component documentation, add a sibling `metadata.ts` file next to a component:
...
Then run `registry:sync` again and review the catalog.
...
- `packages/ui` catalog metadata is generated by `pnpm --filter @repo/ui run registry:sync`.
