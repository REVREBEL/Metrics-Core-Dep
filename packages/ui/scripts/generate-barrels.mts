...
function isTypeDeclaration(file: string): boolean {
...
async function buildIndexContent(dir: string): Promise<string | null> {
  const entries = await fs.readdir(dir, { withFileTypes: true })
...
async function generateBarrels(options: Options) {
...
async function maybeWriteExports(root: string, check: boolean) {
...
async function main() {
  ...
  console.log(
    `[barrels] Updated ${barrelResult.changed} index files (${barrelResult.created} created)`
  )
