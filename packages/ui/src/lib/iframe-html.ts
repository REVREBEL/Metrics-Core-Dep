...
<style id="__runtime-theme-vars"></style>
<style>
  #__error {
    position: fixed;
    inset: 0;
...
window.addEventListener("message", async (e) => {
  ...
  if (e.data.type === "theme-vars") {
    const runtimeThemeVars = document.getElementById("__runtime-theme-vars");
    if (runtimeThemeVars) {
      runtimeThemeVars.textContent = e.data.cssText
        ? ":root {\\n" + e.data.cssText + "\\n}"
        : "";
    }
  }
