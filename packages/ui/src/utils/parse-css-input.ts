...
const processColorValue = (value: string): string => {
	...
	if (cleanValue.startsWith("oklch")) {
		return cleanValue.replace(/\s+/g, " ");
	}
...
const extractBasicShadowProperties = (
	...
	let match = directPropertiesRegex.exec(cssContent);
	while (match !== null) {
		...
		match = directPropertiesRegex.exec(cssContent);
	}
	...
	match = shadowRegex.exec(cssContent);
	while (match !== null) {
		const value = match[1];

		// Check for commas outside of parentheses (multiple shadow layers)
	...
	const bestShadow =
		shadowValues
			.filter((s) => !s.isMultiLayer) // Prefer shadows without top-level commas (single layer)
			.sort((a, b) => {
	...
	// Process the selected shadow. If it's multi-layer (fallback), we take the first layer.
	// We use a similar top-level comma logic to split safely.
	let firstLayer = bestShadow.value;
	let depth = 0;
	for (let i = 0; i < bestShadow.value.length; i++) {
		if (bestShadow.value[i] === "(") depth++;
		if (bestShadow.value[i] === ")") depth--;
		if (bestShadow.value[i] === "," && depth === 0) {
			firstLayer = bestShadow.value.substring(0, i);
			break;
		}
	}
...
const formatOpacity = (opacityStr: string): string => {
	...
	if (cleanOpacity.includes(".")) {
		return Number.parseFloat(cleanOpacity).toString();
	}

	// If opacity is in percentage format (e.g., 50%)
	if (!Number.isNaN(Number.parseInt(cleanOpacity, 10))) {
		return (Number.parseInt(cleanOpacity, 10) / 100).toString();
	}
