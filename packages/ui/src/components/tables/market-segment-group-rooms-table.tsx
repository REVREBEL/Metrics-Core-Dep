...
export function MarketSegmentGroupRoomsTable({
	year,
	month,
	...
	const columns = [
		...
		columnHelper.accessor("occupancy", {
			header: ({ column }) => (
				<DataGridColumnHeader title="Occupancy" column={column} />
			),
			cell: ({ getValue }) => {
				const value = Number.parseFloat(String(getValue() || 0));
				return (
					<div>
						...
						{/* Divide by 100 because 'percent' style expects 0.75 for 75% */}
					</div>
				);
			},
			size: 180,
			...
			filterFn: (row, columnId, filterValue) => {
				...
				const value = Number.parseFloat(String(row.getValue(columnId) || 0));
		...
		columnHelper.accessor("aDR", {
			header: ({ column }) => (
				<DataGridColumnHeader title="ADR" column={column} />
			),
			cell: ({ getValue }) => {
				const value = Number.parseFloat(String(getValue() || 0));
				return (
			...
			filterFn: (row, columnId, filterValue) => {
				if (!filterValue) return true;
				const [min, max] = filterValue;
				const value = Number.parseFloat(String(row.getValue(columnId) || 0)); // Fixed syntax here
				return value >= min && value <= max;
			},
		}),
