...
const MARKET_MIX_DATA = [
	...
	{ name: "Crew", value: 8, segment: "crew", fill: "var(--color-crew)" },
	{
		...
		fill: "var(--color-complimentary)",
	},
	{
		name: "Other",
		value: 7,
		segment: "other",
		fill: "var(--color-other, var(--base-grey-l700))",
	},
];
				...
				<header className="daily-pickup__header">
					...
					<div className="daily-pickup__summary">
						<div className="daily-pickup__summary-top">
							<div>
								<div className="daily-pickup__month">
									{month} {year}
								</div>
							...
							<div className="daily-pickup__mix">
								<div className="daily-pickup__mix-copy">
									<span className="daily-pickup__mix-title">
										Market Mix OTB
									</span>
									<div className="daily-pickup__legend">
										{MARKET_MIX_DATA.map((item) => (
											<span
												key={item.name}
												className="daily-pickup__legend-label"
						...
						<table className="daily-pickup__table">
							<thead>
								<tr className="daily-pickup__table-group-row">
									...
									<th
										colSpan={6}
										className="daily-pickup__section-label--performance"
									>
										Daily Performance Metrics (OTB)
									</th>
									<th
										colSpan={8}
										className="daily-pickup__section-label--pickup"
									>
										Velocity Delta (Pickup)
									</th>
								</tr>
								<tr className="daily-pickup__table-header-row">
									...
									<th>Revenue</th>
									<th>RevPAR</th>
									{["01d", "03d", "07d", "14d", "21d", "30d", "60d", "90d"].map(
										(pu) => (
											<th key={pu}>{pu}</th>
										),
									)}
							...
							<tbody>
								{DAILY_DATA.map((row) => (
									<tr key={`${row.dow}-${row.date}`}>
										<td
											className={
												row.dow === "Sat" || row.dow === "Sun"
													? "daily-pickup__table-number"
													: "daily-pickup__muted"
											}
										>
											{row.dow}
										...
										<td
											className="daily-pickup__event"
											title={row.eventsCy || undefined}
										>
											{row.eventsCy || "—"}
										</td>
										<td className="daily-pickup__event daily-pickup__muted">
											{row.eventsPy || "—"}
										</td>
										...
										<MarketMixBar
											...
											group={row.group}
											crew={row.crew}
											complimentary={row.comp}
											other={row.other}
										/>
										<td className="daily-pickup__table-number">
											${row.adr.toFixed(2)}
										</td>
										<td className="daily-pickup__table-number">
